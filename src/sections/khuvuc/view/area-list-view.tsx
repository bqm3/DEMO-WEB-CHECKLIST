...
export default function AreaListView() {
  ...
  const handleDeleteRow = useCallback(
    async (id: string) => {
      await axios
        .put(`https://checklist.pmcweb.vn/demo/api/ent_khuvuc/delete/${id}`, [], {
          headers: {
  ...
  const handleDeleteRows = useCallback(async () => {
    ...
    await axios
      .put(`https://checklist.pmcweb.vn/demo/api/ent_khuvuc/delete-mul`, deleteRows, {
        headers: {
          Accept: 'application/json',
          Authorization: `Bearer ${accessToken}`,
        },
  ...
  console.log( khuvuc?.map((item) =>
    item.ent_khuvuc_khoicvs.some((i) =>`${ i.ID_KhoiCV}` === '2')
  ).length)
