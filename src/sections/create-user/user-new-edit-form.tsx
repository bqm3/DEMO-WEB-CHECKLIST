  ...
  const onSubmit = handleSubmit(async (data) => {
    try {
      if (currentUser !== undefined) {
        await axios
          .put(`https://checklist.pmcweb.vn/demo/api/ent_user/update/${currentUser?.ID_User}`, data, {
            headers: {
        ...
        if (toggleChecklist === true) {
          ...
          await axios
            .post(`https://checklist.pmcweb.vn/demo/api/ent_user/register`, data, {
              headers: {
                Accept: 'application/json',
                Authorization: `Bearer ${accessToken}`,
              },
    ...
    <Grid xs={12} md={12}>
      <Card sx={{ p: 3 }}>
        ...
        >
          ...
          {`${currentUser?.ID_Chucvu}` === '2' && <RHFTextField name="Password" label="Mật khẩu" />}
        </Box>
